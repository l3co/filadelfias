...
import { getExpenseCategoryLabel } from '../lib/expensePresentation';
...
const DEFAULT_ROW_HEIGHT = 144;
const EXPANDED_ROW_HEIGHT = 230;
const MAX_LIST_HEIGHT = 420;

interface PendingExpenseRowData {
    isApproving?: boolean;
    onApprove: (recordId: string) => void;
    records: ExpenseRequest[];
    rejectReason: string;
...
function PendingExpenseRow({
    ...
    setRejectingId,
    onRejectConfirm,
}: RowComponentProps<PendingExpenseRowData>) {
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">
                                {getExpenseCategoryLabel(record.category)} • {formatDateBR(record.expense_date)}
                            </p>
                            {record.notes && (
...
export function PendingExpensesList({
    ...
    const rowHeight = useCallback((index: number, rowProps: PendingExpenseRowData) => (
        rowProps.rejectingId === rowProps.records[index].id ? EXPANDED_ROW_HEIGHT : DEFAULT_ROW_HEIGHT
    ), []);

    const rowProps = useMemo<PendingExpenseRowData>(() => ({
        isApproving,
        onApprove,
        records,
        rejectReason,
    ...
    }), [handleReject, isApproving, onApprove, records, rejectReason, rejectingId]);
    ...
    return (
        <Card>
            ...
            <CardContent>
                {shouldVirtualize ? (
                    <List
                        rowComponent={PendingExpenseRow}
