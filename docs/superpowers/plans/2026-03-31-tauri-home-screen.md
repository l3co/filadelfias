# HomeScreen (Tela de Início) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Criar a tela de início do app Tauri com versículo do dia, layout para não autenticados (descrição + cards de recursos + CTA) e layout para autenticados (saudação + próximos eventos + devocional de hoje).

**Architecture:** Componente único `HomeScreen` que lê `isAuthenticated` / `isLoading` do `useAuthStore` e renderiza seções condicionais. Versículo do dia gerado localmente (array de 30 versículos, índice por dia do ano) — sem chamada de API, funciona offline. Dados de eventos e devocional só são buscados quando autenticado.

**Tech Stack:** React, React Router v7, TanStack Query, date-fns, lucide-react, Zustand (`useAuthStore`), serviços existentes (`eventsService`, `devotionalsService`).
...
| `apps/tauri/src/routes/index.tsx` | Modificar (1 linha) |

---

### Task 1: Criar HomeScreen — versículo do dia + layout não autenticado

**Files:**
- Create: `apps/tauri/src/routes/public/HomeScreen.tsx`

- [ ] **Step 1: Criar o arquivo com o array de versículos e o layout não autenticado**

```tsx
// apps/tauri/src/routes/public/HomeScreen.tsx
...
export function HomeScreen() {
  ...
  const churchName = (user as { churches?: { name: string }[] } | null)?.churches?.[0]?.name ?? "";
...
Esperado: `✓ built in X.XXs` sem erros de TypeScript.

- [ ] **Step 3: Commit**

```bash
git add apps/tauri/src/routes/public/HomeScreen.tsx
git commit -m "feat(tauri): criar HomeScreen com versículo do dia e layouts auth/guest"
```

---

### Task 2: Registrar HomeScreen na rota `/`

**Files:**
- Modify: `apps/tauri/src/routes/index.tsx:39-53`

- [ ] **Step 1: Adicionar o import e substituir o Placeholder**

No topo do arquivo, adicionar o import após os outros imports de `public/`:
...
```bash
git add apps/tauri/src/routes/index.tsx
git commit -m "feat(tauri): registrar HomeScreen na rota raiz"
```
