...
class User(Base):
    ...
    memberships: Mapped[list["UserChurchMembership"]] = relationship(
        back_populates="user", foreign_keys="UserChurchMembership.user_id", cascade="all, delete-orphan"
    )
...
class Tenant(Base):
    ...
    councils: Mapped[list["Council"]] = relationship(back_populates="tenant", cascade="all, delete-orphan")
    accounts: Mapped[list["FinancialAccount"]] = relationship(back_populates="tenant", cascade="all, delete-orphan")
    categories: Mapped[list["TransactionCategory"]] = relationship(
        back_populates="tenant", cascade="all, delete-orphan"
    )
    missionaries: Mapped[list["Missionary"]] = relationship(back_populates="tenant", cascade="all, delete-orphan")
    ebd_classes: Mapped[list["EBDClass"]] = relationship(back_populates="tenant", cascade="all, delete-orphan")
...
class Member(Base):
    ...
    office: Mapped[str] = mapped_column(
        String(50), default="MEMBRO", nullable=False
    )  # MEMBRO, DIACONO, PRESBITERO, PASTOR
    ...
    )  # PROFISSAO_FE, TRANSFERENCIA, JURISDICAO
